...
export class PixController {
    ...
    async handleEfiWebhook(
        ...
        try {
            await this.efiPixService.handleWebhook(efiPayload);
            return 'OK';
        } catch (error) {
    ...
    @Post('configure-webhook') // Exemplo de rota
         ...
         try {
             const result = await this.efiPixService.configureEfiWebhook();
              return {
                  ...
                  efiResponse: result // Inclui a resposta da Efí
              };
         } catch (error) {
              this.logger.error(`Erro ao configurar webhook via endpoint: ${(error as any).message}`, (error as any).stack);
              throw new InternalServerErrorException(`Falha na configuração: ${(error as any).message}`);
         }
    }
    // --- Fim NOVO ENDPOINT ---
}